(() => {
  ...
  function actuatorMarkup() {
    return `
      <span class="actuator__bezel"></span>
      <span class="actuator__recess"></span>
      <span class="actuator__ring"></span>
      <span class="actuator__cap"></span>
      <span class="actuator__gloss"></span>
    `;
  }

  function buildRedemptionLayer() {
    if (matrix.querySelector('.matrix-redemption')) {
      return matrix.querySelector('.matrix-redemption');
    }
  ...
  function resetRedemption() {
    ...
    continuityActuator.style.transform = 'scale(0.35)';
  ...
  function runSequence() {
    ...
    setCopy(
      'Object detected · magnetic capture',
      'MAGNETIC CAPTURE · OBJECT DETECTED',
      'ALIGNING FV–01',
    );
    ...
    later(() => {
      setCopy(
        'Registering matrix · FV–01',
        'AMBER VERIFICATION · REGISTERING MATRIX',
        'DO NOT REMOVE',
      );
    }, 290, token);
    ...
    later(() => {
      ...
      setCopy(
        'Matrix redeemed · FV–01',
        'MATRIX REDEEMED · ACCESS CHIP RELEASED',
        'OBJECT INTAKE',
      );
    }, 780, token);
    ...
    later(() => {
      ...
      animate(
        ...
        [
          {
            opacity: 0,
            transform: `translate(${translateX}px, ${translateY}px) scale(.42)`,
          },
          {
            opacity: 1,
            transform: `translate(${translateX}px, ${translateY}px) scale(.42)`,
  ...
  const stageObserver = new MutationObserver(() => {
    const stage = portal.dataset.stage;
    ...
    if (stage === 'receiving' && active) {
      resetRedemption();
    }
  });

  stageObserver.observe(portal, {
    attributes: true,
    attributeFilter: ['data-stage'],
  });

  window.addEventListener('pageshow', (event) => {
    if (event.persisted || portal.dataset.stage === 'receiving') {
      resetRedemption();
    }
  });
